...
for E in "${!ar_sh[@]}"; do
   if ! [[ -e ${ar_sh[E]/.\/} ]]; then
      f_no "${ar_sh[E]/.\/}"
      printf "${_inf} try get ${_CYN} from origin" "${ar_sh[E]/.\/}"
...
for e in "${!ar_zh[@]}"; do
   if ! [[ -e ${ar_zh[e]} ]]; then
      f_no "${ar_zh[e]}"
      printf "${_inf} try get ${_CYN} from origin" "${ar_zh[e]}"
...
for G in {8..11}; do
   f_sm7 "$G" "${ar_sho[G]}"; f_add "${ar_url[G]}" | _grep -v "^#" >> "${ar_dmn[3]}"; f_do
done
...
f_sm7 12 "${ar_sho[12]}"
f_add "${ar_url[12]}" | _grep -v "^\(#\|:\)" | cut -d' ' -f2 >> "${ar_dmn[2]}"; f_do
f_sm7 13 "${ar_sho[13]}"
f_add "${ar_url[13]}" | _sed "1,11d;/^;/d" | cut -d' ' -f1 >> "${ar_dmn[2]}"; f_do

for H in {14..18}; do
   f_sm7 "$H" "${ar_sho[H]}"; f_add "${ar_url[H]}" | _grep -v "#" >> "${ar_dmn[2]}"; f_do
done
...
until [[ ${RETVAL} =~ ^[1-4]{1}$ ]]; do
   printf "please enter: ${_CYN} or Ctrl+C to quit\n" "[1|2|3|4]"
   read -r RETVAL
done
