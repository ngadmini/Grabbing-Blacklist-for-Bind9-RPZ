...
for C in {"$_DPL","$_BLD","$_CRL","$_SCP"}; do [ -e "$C" ] || f_xcd 17 "$C"; [ -x "$C" ] || chmod +x "$C"; done
...
for H in ${ar_dmn[0]}; do f_falsf "${ar_cat[0]}" "$H" "${ar_txt[0]}" "${ar_reg[0]}" "${ar_reg[1]}"; done
...
for K in ${ar_dmn[4]}; do f_falsf "${ar_cat[4]}" "$K" "${ar_txt[4]}" "${ar_reg[0]}" "${ar_reg[1]}"; done
...
for N in ${ar_dmn[3]}; do f_falsf "${ar_cat[3]}" "$N" "${ar_txt[3]}" "${ar_reg[0]}" "${ar_reg[1]}"; done
...
for Q in ${ar_dmn[2]}; do f_falsf "${ar_cat[2]}" "$Q" "${ar_txt[2]}" "${ar_reg[0]}" "${ar_reg[1]}"; done
